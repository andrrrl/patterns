...
PuntoCSS.prototype = {
    ...
    cargar: function(bordado_id, opciones) {
        ...
        Bordado.data.id && $.ajax({
            ...
            error: function() {
                return console.info("[ERROR] $.ajax: Error al cargar el bordado: " + Bordado.data.bordado), 
                !1;
            }
        });
    },
    guardar: function(bordado_nombre) {
        ...
        var Bordado = this;
        if (Bordado.data) {
            var btn_save = this.btn_save, btn_states = {
                      ...
                      case "ok":
                        ...
                        $("#lista-bordados-borrar").find("li a").on("click", function(e) {
                            ...
                            if (console.log("Bordado.data.id: " + id), $(this).hasClass("delete")) {
                                var delete_id = $(this).data("delete-id"), bordado = $(this).data("bordado");
    ...
    eliminar: function(id) {
        return "undefined" == typeof id ? (console.info("No se pudo eliminar el bordado: se requiere un ID..."), 
        !1) : (console.info("$.ajax: Eliminando el bordado..."), void $.ajax({
            ...
            success: function(res) {
                switch (res.result) {
                  ...
                  case "error":
                    return console.info("$.ajax: No se pudo eliminar el bordado..."), console.log(res), 
                    !1;
                }
            },
    ...
    grilla: function(opciones) {
        ...
        var filas = parseInt(this.data.filas), columnas = parseInt(this.data.columnas), ancho = parseInt(this.data.ancho_punto);
        ...
        for (var y = 0; y < this.data.filas; y++) for (var x = 0; x < this.data.columnas; x++) primera_linea = 0 === x ? " primera-linea" : "", 
    ...
    agregar: function(punto) {
        var punto_nuevo = punto.coord.x + "," + punto.coord.y, coords = this.data.coords.map(function(e) {
            return e.coord.x + "," + e.coord.y;
        }), color_hilo = punto.color_hilo, ancho_hilo = punto.ancho_hilo, tipo_punto = punto.punto;
    ...
    quitar: function(coord) {
        for (var i = 0; "undefined" != typeof this.data.coords[i]; ) {
