...
shell.echo('╭──────────┤  ▶ ' + appname + ' Postinstall  ├──────────╮'.bold.yellow);
shell.echo('│          ╰─────────────────────────╯          │'.bold.yellow);

shell.echo(' [INFO] Installing Velours...'.bold);

let wd = shell.pwd().stdout;
wd = wd.slice(wd.lastIndexOf('/'));

if ( wd === '/utils' && process.argv[2] !== 'test' ) {
...
if ( env_found ) {
	...
	shell.echo(( ' [OK] ' + appname + ' installed successfully!' ).green.bold);
	shell.echo(( ' [INFO] Run `npm start` and optionally `grunt` (for dev) from ' + appname + '\'s root dir' ).bold);
